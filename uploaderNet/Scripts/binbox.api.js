if (typeof sjcl == 'undefined' || typeof sjcl.decrypt != 'function') {
    "use strict";
    var sjcl = {
        cipher: {},
        hash: {},
    ...
    sjcl.random = {
        ...
        U: function (a) {
            ...
            for (d =
            0; d < 16; d++) b.push(Math.random() * 0x100000000 | 0);
...
/*
 * $Id: base64.js,v 2.1.2 2011/12/27 14:34:49 dankogai Exp dankogai $
 *
...
var Binbox = {
    ...
    widgets: function () {
        $('.binbox:not(.ready)').each(function () {
            $(this).addClass('ready');
            ...
            if ($(this).hasClass('paste')) {
                $(this).replaceWith("<iframe style='width: " + data.width + "px;height: " + data.height + "px;border:0;' src='" + Binbox.URL + "" + $(this).data('paste') + "/widget?" + $.param(data) + "#" + ($(this).data('password') ? $(this).data('password') : '') + "'></iframe>");
            } else if ($(this).hasClass('bin')) {
                if ($(this).hasClass('inline')) {
                    $(this).replaceWith("<iframe src='" + Binbox.URL + "widget?" + $.param(data) + "' style='width: " + data.width + "px;height: " + data.height + "px;border:0;'></iframe>");
                } else {
                    $(this).click(function (e) {
                        e.preventDefault();
                        window.open(Binbox.URL + "widget?" + $.param(data), "New Paste", "width=" + data.width + ",height=" + data.height + ",location=0,menubar=0,scrollbars=0,status=0");
                    });
                }
                ...
                size = sizes.hasOwnProperty($(this).data('size')) ? $(this).data('size') : 'small';


                data.width = sizes[size][0];
                data.height = sizes[size][1];

                $(this).replaceWith("<iframe scrolling='no' style='width: " + data.width + "px;height: " + data.height + "px;border:0;' src='" + Binbox.URL + "/widget/badge?" + $.param(data) + "'></iframe>");
            }
        });
    },
    ...
    API: function (API_URL) {
        ...
        this.create = function (options, callback) {
            ...
            var res = this.encrypt(options.text);
            console.log(res.data);
            console.log(res.salt);
            console.log(API_URL);

            $.post(this.URL + "submit.json", {
                'data': res.data,
                'title': options.title
            }, function (result) {
                console.log(this.URL + " ");
                result.salt = res.salt;
                console.log(res.salt + " ");
                callback(result);
            }, 'json');
        };
        ...
        this.retrieve = function (id, callback) {
            ...
            $.get(this.URL + id + ".json", function (result) {
                if (!result.ok) {
                    callback(false);
                    return false;
                }

                result.decrypt = function (salt) {
                    return sjcl.decrypt(salt, this.paste['text']);
                }

                if (salt) {
                    callback(result, result.decrypt(salt));
                } else {
                    callback(result);
                }
            }, 'json');
        ...
        this.encrypt = function (text, salt) {
            if (!salt) {
                salt = this.salt(8);
                var data = sjcl.encrypt(salt, text);
        ...
        this.decrypt = function (text, salt) {
            return sjcl.decrypt(salt, text);
        };
        this.salt = function (charlength) {
...
Binbox.log = function () {
    console.log("Binbox API - ", arguments);
    //alert("Binbox API - " + arguments.join(" "));
    typeof console.log == 'function' ? console.log("Binbox API - ", arguments) : alert("Binbox API - " + arguments.join(" "));
}


Binbox.load_pastes = function () {

    function unlock(result, password, box) {
        function entry() {
            var unlocker = $("<div class='binbox-decrypt'><form action='' method='post'><div class='binbox-title-container'>Enter the password for the paste #" + result.paste['id'] + "</div><div class='binbox-password-container'><input type='text' name='password' /></div><div class='binbox-unlock-container'><button>Unlock</button></form></div>");

            unlocker.submit(function (e) {
                e.preventDefault();
                unlock(result, unlocker.find('input[name="password"]').val(), box);
            });
        ...
        var json = $("<div></div>").text(result.paste['text']).text();
        console.log(json);

        try {
            Binbox.log(json);
            var text = sjcl.decrypt(json, password);
            if (text) {
                $(box).html($("<pre class='binbox-paste-text'></pre>").text(text));
            }
        } catch (e) {
            Binbox.log(e);
            entry();
    ...
    $('.binbox').each(function () {
        ...
        $.get("http://binbox.io/" + $(this).data('id') + ".json", function (result) {

            if (result.ok && typeof result.paste != 'undefined') {
                unlock(result, $(this).data('password'), box, 0);
            } else {
                $(box).html("<iframe src='http://binbox.io/ad/300x250' style='width:300px;height:300px;border:0;'></iframe>");
            }
